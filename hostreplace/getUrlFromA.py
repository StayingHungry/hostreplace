...
#此函数用来判断页面中是否包含不想要的元素
def getFromPage_judge(element):
    js = []
    ...
    for url in urls:
        ...
        element_count = content_page.lower().count(element)
        print "this url contains " + str(element_count) + "  " + element


##此函数用来判断a标签里面是否包含不想要的元素
def getFromA_jude(element):
    js = []
    js0 = 'var x=document.getElementsByClassName("ad_result");if(x.length>0){for(var i=0;i<x.length;i++)x[i].parentNode.removeChild(x[i])};'#删除广告
    js1 = 'document.getElementById("kmap-right-container").remove()'  #删除相关内容
    js.append(js0)
    ...
    for url in urls:
        ...
        for abiaoqian in suoyouabiaoqian:
            acontent = abiaoqian.get_attribute("outerHTML").lower()  #outterHTML就是整个a标签
            if element in acontent:
                print "warning!!!!"
            else:
                print acontent
...
#用此函数判断url中是否含有不想要的元素
def getFromAUrl_jude(element):
    js = []
    js0 = 'var x=document.getElementsByClassName("ad_result");if(x.length>0){for(var i=0;i<x.length;i++)x[i].parentNode.removeChild(x[i])};'#删除广告
    js1 = 'document.getElementById("kmap-right-container").remove()'  #删除相关内容
    js.append(js0)
    ...
    for url in urls:
        ...
        if index< 540:
            continue
        print "zheshidi " + str(index) + "ge url"
        driver.get(url.strip())
        time.sleep(0.2)
        for js_code in js:
            try:
        ...
        for abiaoqian in suoyouabiaoqian:
            try:
                urlcontent = abiaoqian.get_attribute("href").lower()
                if element in urlcontent:
                    if "m/web/uid" not in urlcontent:
                        print "waring!!!!" + urlcontent
                    else:
                        if "http%3a%2f%2fbaike.sogou.com"  in urlcontent[urlcontent.find("url="):urlcontent.find("url=")+40]:
                            continue
                        # elif "http%3a%2f%2fwenwen.sogou.com" in urlcontent[urlcontent.find("url="):urlcontent.find("url=")+40]:
                        #     continue
                        elif "sogou.com" in urlcontent[urlcontent.find("url="):]:
                            print "waring!!!!" + urlcontent
...
def getUrlFromA():
    js = []
    js0 = 'var x=document.getElementsByClassName("ad_result");if(x.length>0){for(var i=0;i<x.length;i++)x[i].parentNode.removeChild(x[i])};'#删除广告
    js1 = 'document.getElementById("kmap-right-container").remve()'  #删除相关内容
    js.append(js0)
    js.append(js1)
    for url in urls:
        print "****"*10
            ...
            try:
                ...
                try:
                    ...
                    if ".com" not in acontent:
                        ...
                        try:
                            print requests.get(acontent,headers=my_headers, timeout = 0.5).status_code
                        except:
...
# getFromA_jude("sogou.com")
# getUrlFromA()
# getHost('https://wap.sogou.com/web/searchList.jsp?uID=V5npvumGTk-3uIl0&v=5&dp=1&w=1278&t=1508233127824&s_t=1508233130135&s_from=result_up&htprequery=c&keyword=%e5%9c%a3%e5%a2%9f')


# getFromPage_judge("sogou.com")
getFromAUrl_jude("sogou.com")
# getUrlFromA()
urls_file.close()
driver.close()
driver.quit()
